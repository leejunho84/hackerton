...
(function($, ns){
	...
	Swipe.prototype = {
		init:function(){
			_that = this;
			_that.duration = ns.support.transforms3d || ns.support.transforms ? _that.opts.duration * 0.001 : _that.opts.duration;
		...
		addEvent:function(){
			if(ns.support.touch){
				ns.support.addEvent(_that.$container[0], 'touchstart', onTouchStart);
				ns.support.addEvent(_that.$container[0], 'touchmove', onTouchMove);
				ns.support.addEvent(_that.$container[0], 'touchend', onTouchEnd);
				ns.support.addEvent(_that.$container[0], 'touchcancel', onTouchCancel);
			}else{
				ns.support.addEvent(_that.$container[0], 'mousedown', onTouchStart);
			}
		},
		addIndicator:function(){
			...
			_that.$indicator.each(function(i){
				ns.support.addEvent($(this)[0], 'click', function(e){
		...
		destroy:function(){
			if(ns.support.touch){
				ns.support.removeEvent(_that.$container[0], 'touchstart', onTouchStart);
				ns.support.removeEvent(_that.$container[0], 'touchmove', onTouchMove);
				ns.support.removeEvent(_that.$container[0], 'touchend', onTouchEnd);
				ns.support.removeEvent(_that.$container[0], 'touchcancel', onTouchCancel);
			}else{
				ns.support.removeEvent(_that.$container[0], 'mousedown', onTouchStart);
			}
		},
	...
	function slideForTransition($target, index, fps){
		if(ns.support.transforms3d || ns.support.transforms){
			if(ns.support.transforms) _that.translate = 'translate(' + _that.arrPos[index] + 'px, 0)';
			if(ns.support.transforms3d) _that.translate = 'translate3d(' + _that.arrPos[index] + 'px, 0, 0)';
			$target.css({
	...
	function onTouchStart(e){
		var touchobj = (ns.support.touch) ? e.touches[0] : e;
		_that.startX = touchobj.clientX;
		_that.startY = touchobj.clientY;

		if(!ns.support.touch){
			ns.support.addEvent(document, 'mousemove', onTouchMove);
			ns.support.addEvent(document, 'mouseup', onTouchEnd);
			
			if(ns.agentChk.getBrowser().browser == 'IE' && ns.agentChk.getBrowser().version < 9){
				e.returnValue = false;
				e.cancelBubble = true;
			}else{
				e.preventDefault();	
	...
	function onTouchMove(e){
		...
		for(var i=0; i<_that.swipeLen; i++){
			...
			if(ns.support.transforms3d || ns.support.transforms){
				if(ns.support.transforms) translate = 'translate(' + (_that.arrPos[posIndex] - _that.touchPos) + 'px, 0)';
				if(ns.support.transforms3d) translate = 'translate3d(' + (_that.arrPos[posIndex] - _that.touchPos) + 'px, 0, 0)';
		...
		if(Math.abs(_that.distX) < Math.abs(_that.distY) && _that.bubbleIS){
			if(ns.support.touch) ns.support.removeEvent(_that.$container[0], 'touchmove', onTouchMove);
		}else{
	...
	function onTouchEnd(e){
		...
		if(ns.support.touch){
			...
			ns.support.removeEvent(document, 'mousemove', onTouchMove);
			ns.support.removeEvent(document, 'mouseup', onTouchEnd);
